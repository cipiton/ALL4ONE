...
class SkillMdAdapter(SkillAdapter):
    ...
    def find_resume_point(self, outputs_root: Path) -> SkillResumePoint | None:
        ...
        if state.status == "completed_step" and state.detected_step < self._skill_definition.final_step_number:
            next_step = state.detected_step + 1
    ...
    def resume(self, request: SkillResumeRequest) -> SkillRunResult:
        ...
        resume_document, resumed_step_hint = load_resume_document(self._skill_definition, state)
        session_dir, results = execute_input_paths(
            request.repo_root,
            self._skill_definition,
